...
<Aside type="note">
  Phase 1 (account context system) is implemented. See the
  [Multi-Credential Management guide](/guides/multi-credential-management/) for usage
  and the [Account Security Model](/architecture/account-security-model/) for the security
  architecture. Phases 2 and 3 (workstream spawner, budget enforcement) remain proposals.
</Aside>
...
Your Terminal (Personal Token — Max Plan)
  └── claude (your interactive session)
       └── reagent serve (MCP server — middleware chain)
            │
