...
`reagent init` configures your repository with security hooks, behavioral policies, quality gates, agent teams, and developer tooling. It also installs `@bookedsolid/reagent` as a devDependency (detecting your package manager and workspace roots automatically).

## What gets installed

| Path                         | Committed       | Purpose                                      |
| ---------------------------- | --------------- | -------------------------------------------- |
| `.mcp.json`                  | Yes             | MCP server declaration for Claude Code       |
| `.reagent/policy.yaml`       | Yes             | Autonomy levels and agent policy             |
| `.reagent/gateway.yaml`      | Yes             | Downstream MCP server proxy config           |
...
| `.claude/hooks/`             | No (gitignored) | Claude Code safety hooks (26 scripts)        |
...
| `--base-profile <name>`   | Base profile for tech stack profiles           | `client-engagement` |
...
Restart Claude Code after running `reagent init`. Claude Code reads `.mcp.json` on session
start and spawns `reagent serve` automatically via stdio transport.

### Package manager detection

`reagent init` detects your package manager from lockfiles and installs itself as a devDependency:

| Lockfile              | Package manager | Workspace flag  |
| --------------------- | --------------- | --------------- |
| `pnpm-lock.yaml`      | pnpm            | `-w` (if workspace root) |
| `yarn.lock`           | yarn            | `-W` (if workspace root) |
| `bun.lockb`           | bun             | —               |
| `package-lock.json`   | npm             | —               |

For workspace monorepos, init detects `pnpm-workspace.yaml` or `workspaces` in `package.json`
and adds the appropriate workspace flag so the dependency installs at the root.
